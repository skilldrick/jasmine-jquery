...
(function(namespace) {
  ...
  function argumentsArrayContainsArg(argumentsArray, expectedArgument, env) {
    var found = false;
    //iterate over the event arguments
    $.each(argumentsArray, function (index, args) {
      if (env.contains_(args, expectedArgument)) {
        found = true;
      }
    });
    return found;
  ...
  namespace.events = {
    ...
    wasTriggered: function(selector, eventName, matcherClass) {
      selector = convertJQueryToSelector(selector);
      matcherClass.selector = selector;
      matcherClass.eventName = eventName;
      return data.spiedEvents[[selector, eventName]].length > 0;
    },

    withArg: function(expectedArg, matcherClass, env) {
      var actualArgs = data.spiedEvents[[matcherClass.selector, matcherClass.eventName]];

      return argumentsArrayContainsArg(actualArgs, expectedArg, env);
    },
...
beforeEach(function() {
  ...
  this.addMatchers({
    toHaveBeenTriggeredOn: function(selector) {
      ...
      return jasmine.JQuery.events.wasTriggered(selector, this.actual, this);
    },
    ...
    withArg: function (arg) {
      return jasmine.JQuery.events.withArg(arg, this, this.env);
    },
...
var oldMatchersFn = jasmine.Matchers.matcherFn_;
jasmine.Matchers.matcherFn_ = function (matcherName, matcherFunction) {
  //console.log(this);
  var oldRetVal = oldMatchersFn.apply(this, arguments);
  return function (matcherName, matcherFunction) {
    console.log(oldRetVal);
    return oldRetVal;
  }.bind(this);//
  //return oldMatchersFn.apply(this, arguments);
};
